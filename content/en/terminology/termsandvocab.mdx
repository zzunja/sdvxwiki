...
## Techniques
    This section refers to non-cheesing techniques only.
    
    #### Vertical Reading
    Vertical reading is dividing a patterns into multiple "lanes" (not to be confused with a gameplay component), or vertical sections, and reading them separately. 

    <Image src="/terminology/terminology/verticalreading.png"/>
    <em className="flex justify-center"> An example from WHITEOUT MXM20</em>

    In this example, dividing the pattern into two "left" and "right" sides can be beneficial since you can hit the [16th notes](#n-th-notes) on the "left" side and [12th notes](#n-th-notes) on the "right" side independently. Trying to read the pattern normally is harder since these notes are essentially [48th](#n-th-notes), therefore a lot denser and faster.

    #### Horizontal Reading
    Horizontal reading is the contrast of Vertical Reading - diving a pattern into multiple horizontal sections, and reading them one-by-one as the track goes.

    <Image src="/terminology/terminology/horizontalreading.png"/>
    <em className="flex justify-center"> An example from Blastix Riotz EXH 18</em>

    In this example, dividing the pattern into multiple sections starting with each fx notes can significantly help with timing. This way, players can maintain the overall rhythm even if some of the notes in each section go off-rhythm. Otherwise, one mis-timed note can lead to all the following notes going off-rhythm.
