...
Even though they look different, they are playing the same chart. It isn't "faster" or "slower" chart. 
...
There are 2 offsets. Draw (`判定タイミング(音)`) and Judge (`判定タイミング(描画)`). Draw is visual offset and Judge is audio offset.
Draw moves the judgment line either up (+) or down (-). It doesn't do this
visually, the judgment line will still look like it's in the same spot but it has moved. Here is a visualizatoin of Draw Offset

<Image src="/startingout/options/draw-example.png" className="w-100"/>

The line at the top would represent where the judgement line would be heading to
for `+` draw. The bottom line would represent where the judgement line would be
heading to for `-` draw. The line inbetween is the default judgement line.

Judge just adds offset to the music.
...
other show up for all or just crits. That way you know if you hit a near by if one shows up.
...
This setting changes whether or not you can see your best scores overall from Arcade and Konasute. If enabled, it will combined your Arcade and Konasute scores and show you what you have the highest of. So, if you have a of `9,978,132` on Konasute and a `9,942,612` on Arcade, it will display the Konasute score.

Note that Volforce is seperate between the two
...
This changes how the lasers look. Enabling it adds a bit more effect to lasers. For an example, see video below
...
Nears, and Errors. For more information on EX-Scoring see the [Ex-Score page.](/compendium/exscore)
