...
There are 2 offsets. Draw (`判定タイミング(描画)`) and Judge (`判定タイミング(音)`). Judge moves both the judgement line **and** audio while Draw **only** affects the judgement line. While this might seem complicated, you can think of it as Judge affects both visually when to hit the note and the time to hit the note, while Draw only affects visually when to hit the note. 

So, if you were to want only audio offset of `+8ms` with your judgement line at default, you would adjust your Judge to `+8ms` and your Draw `-8ms`.
...
for `+` offset. The bottom line would represent where the judgment line would be
heading to for `-` offset. The line in between is the default judgment line.
...
For a visual representation of what each does, you can use [this video](https://www.youtube.com/watch?v=hp-0K3FR7FU).
...
This option controls whether your current position is displayed during play.
When it is on, the game shows your position in a match and the difference in
score between you and your matching opponent/target score.
Example below.
...
This will not show if you have [Rank Display](#rank-display) turned off, even if
the selected target score is not your matching opponent.
