...
<Image src="/startingout/scoring/normal_score.webp" alt="normal score" />
...
Related to EX SCORE is the `MAX` metric, which is the percentage obtained by dividing your EX SCORE by the theoretical maximum. For percentages above 98%, it is presented as `MAX-X` instead, where `X` is the absolute difference between your EX SCORE and the theoretical maximum.
