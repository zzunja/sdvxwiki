...
There are currently 2 folders for Variant Gate. Here we will talk about the first one.
...
corresponding [Radar](/compendium/effect-radar) of the chart you want to unlock. For
example, if you want to unlock the MXM Sword chart, you would have to play charts with either `One-hand` or `Peak` as their [Radar](/compendium/effect-radar). 

Note that the chart difficulty you pick matters when choosing what to unlock. To
unlock the MXM of the 19s, you have to pick a MXM with the same radar. For
example, if you pick EXH charts with `Notes` as its radar, it will give you **Who then no 灯** since its EXH is a `Notes` radar. 

Unless you have the Blaster Pass, the `VARIANT POWER` bar will only fill up with [MAXXIVE RATE](/compendium/gauge-options#maxxive-rate) clears. Meaning, you have to clear charts with MAXXIVE RATE to fill the Variant Bar.

You can filter by radar using [Radar Folders](/startingout/gameplay-screens#song-select-folders)

[Blaster Start](/compendium/game-modes#blaster-start) gives you 33% on the Variant Bar, so on your extra track, you will
guarantee be able to play one of the charts. If combined with blaster pass, it will give 66%.
...
To access the 20 song, you have to clear all the charts of the previous three charts.
...
If you fail, you will have to redo clearing all the charts again to try again. You
