...
## BLC
[BLC](/terminology/termsandvocab#pcb-blc-pc) (in-game currency) is the simplest way to unlock songs and charts: for songs and charts that can be unlocked with BLC, select the song on the song select screen, press `START`, and confirm the BLC cost to unlock the selected difficulty and all lower difficulties.

The amount of BLC required depends on the version in which the song was released and the chart's level. Unlocking a higher difficulty also unlocks the lower difficulties, so the cost includes those lower difficulties. The amount of BLC earned depends on factors such as song play, PASELI, matching, and events.
...
If you fail to clear the song 20 times, it will automatically unlock no matter if you pass or fail.

## Variant Gate
<Image src="/compendium/unlock-systems/variant_charts.webp"/>

Variant Gate is the unlock system introduced in Sound Voltex: Exceed Gear

To unlock the first 3 songs in the first Variant Gate, you have to fill the
`VARIANT POWER` bar with the corresponding [EFFECT RADAR](/compendium/effect-radar)
attributes of the chart you want to unlock. The 3 songs correspond to these 3
groups of EFFECT RADAR attributes:
- TRICKY and NOTES
- HAND-TRIP and TSUMAMI
- ONE-HAND and PEAK

After clearing a chart, your `VARIANT POWER` will fill according to its EFFECT
RADAR attributes. For example, playing a chart with `NOTES` as its highest
value will fill the TRICKY and NOTES group.

Each group of radar attributes has a corresponding song. For example, in the
first Variant Gate folder, filling the `ONE-HAND` or `PEAK` group unlocks
**Who then no 灯**.

You can filter by radar using [Radar Folders](/startingout/gameplay-screens#song-select-folders)

<div className="mt-2 text-center">
**Without Blaster Pass**
<div className="flex items-center justify-center">
<br/>
|Clear|Standard|Blaster Start|
|:-:|:-:|:-:|
|Complete|+4%|+8%|
...
<div className="mt-2 text-center">
**With [Blaster Pass](/compendium/basic-premium-courses#blaster-pass)**
<div className="flex items-center justify-center">
<br/>
|Clear|Standard|Blaster Start|
|:-:|:-:|:-:|
...
chart if **you are on a EX-TRACK**. The Variant folder only shows on EX-TRACK. There are different [Ranks of difficulty](/compendium/gauge-options#variative-rate) that represent how hard the gauge
...
To access the 20 song, you have to clear the MXM of all the previous three charts.
Upon clearing your chart, you will get a choice of which difficulty you want to play then put into the chart. You will be
locked to the max difficulty of `Rank V`. 

For the first two variant gates (Kannagi and Everlasting Message) your seals will be saved. For others, upon failing your seals will be lost and you will have to get them again by clearing all previous charts on any rank. You
will be able to challenge it at `RANK IV`. If you fail `RANK IV`, next attempt you will be able to attempt at `RANK III` and so on.

You can overfill your VARIANT POWER up to 500% to play charts on a lower
starting rank. Once it reaches 200%, you can start a challenge one rank lower.
All VARIANT POWER is consumed when you start a challenge; any overfill is used
to lower the starting rank on a retry.
...
|Any Battle + Standard Start |650 AP|1300 AP|
...
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|Without Blaster Pass|1.0|1.1|1.2|1.3|1.4|1.5|
|With Blaster Pass|1.5|1.6|1.7|1.8|1.9|2|
</div>
...
|:-:|:-:|
|Blaster Pass|+5|
|Premium Time Start|2x|
|Blaster Start|3x|
|Megamix Start|3x|
</div>

Note that Megamix only counts as one song.
There are also a daily bonus for playing consecutive days and a weekly bonus for consecutive weeks. Daily bonus goes from 1-5, and weekly bonus goes from 5-10.
