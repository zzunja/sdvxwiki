...
# .ksh Format
<Callout>For more details and specifics, you can look at [masaka offical writeup](https://github.com/kshootmania/ksm-chart-format/blob/master/ksh_format.md) on the format</Callout>
...
Here, we have two lines: `beat=4/4` and `0000|00|--|`. The first line is simple; it defines the current [time signature](/terminology/termsandvocab#time-signatures). If the time signature were to change, there would be a `beat=` to signify this change. This goes the same with `t=` (BPM) as well. If there is a BPM change, it will be signified in the file like this. You can change the BPM and time signature as much as you want.
...
<Image src="/charting/kshfile/example4.webp" width={100} height={100} caption="Above example in editor" alt=""/>
...
### Footer
There is also can be a footer in a chart. This is simply where custom effects and filters for FX are defined. These looks like:
```jsx
#define_fx
#define_filter
```
