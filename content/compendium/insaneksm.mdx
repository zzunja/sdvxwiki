...
- [What is Insane KSM?](#what-is-insane-ksm)
  - [Examples of Charts from Insane KSM](#so-why-should-i-play-insane-ksm)
  - [os/us Scale](#osus-scale)
    - [Undershoot or us](#undershoot-or-us)
    - [Overshoot or os](#overshoot-or-os)
...
restrictions. These charts go past the vanilla 1-20 scale. Most charts
...
Normally, certain "illegal" patterns are either really hard,
...
### os/us Scale
Insane KSM uses a different scale than the 1-20 scale present in Sound Voltex. These are the Overshoot (os) and Undershoot (us) scales. The official table for how this translates to SDVX difficulty is this:
...
with a `*` at the end. For example, an overshoot 3 unique would be marked as `os3*`.
...
#### Undershoot or us
Undershoots are below overshoots and are easier than Overshoots. These are often charts that are good for starting.
...
This drive has every single `os0` - `os16` chart that is officially on the v5.0 sheet. It does not include all the extra charts from these packs. This also works as a way to sort Overshoot charts by difficulty.

[comment]: # "replace with my google drive of all charts after it finishes uploading" 
...
of custom charts in the style of Overshoots on [Nautica.](https://ksm.dev/)
Nautica is the place to find custom KSM charts. While most charts are more
normal, there exist overshoot-style charts here as well. 

Charters that are often harder charts are:
