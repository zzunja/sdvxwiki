...
Also, your volforce is calculated as **your best clear and your best score** on a
chart. For example if you get a `9,899,320` with a `UC` but already had a
`9,923,042` with a `Excessive Rate Clear`, it would use the `UC` to calculate the volforce.

With the equation it would look like this
<div className="flex items-center justify-center mt-3">
``(18*(9,923,042 / 10,000,000)*(1.05)*(1.05)*20)*.001``
</div>
Note the `1.02` has changed to a `1.05`, but it used the better score that I originally had. This means 
I would gain .393-.382=**.011 Volforce** for just increasing my Excessive to a UC, even with a worse score.
