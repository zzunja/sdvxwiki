...
本指南示範常見配置下的手部姿態與技巧。SDVX 是一個很「動態」的遊戲，常常需要讓雙手（有時也包含身體）離開預設姿勢。
...
其中一隻手的拇指伸到另一隻手的手掌底下，或者調整姿態使得一隻手完全在前方。之後當作雙手的交互處理。
...
指用張開的手掌轉動旋鈕。這是較差的手法，**不建議使用**，因為旋鈕持續時間較長時會很快沒有空間可以動，此外過大的側向施力可能損壞旋鈕。
...
這些進階技巧適用於高難度譜面（18+）。原本以預設姿勢就能處理的簡單配置，可能會因速度或密度增加而變得困難。影片中的動作為求清楚而有所誇大。
  ...
  此外如 Mini 在其 [文章](https://note.com/mini208/n/n3d77e9743e68) 中所述，把高速階梯當作組合鍵處理時（餡蜜），雙押部份靠後比較容易減少誤差。
  ...
  指單手同時轉動旋鈕與按下按鍵。可用於 `ONE-HAND` 配置變得過於複雜的情況。
