import { Image } from "../../components/Image";
import { Video } from "../../components/Video";

# Hand Positioning

## Table of Contents
- [Intro](#intro)
  - [Standard/Static Position](#standardstatic-position)
- [Positioning](#positioning)
  - [Double FX](#double-fx)
    - [Poor Technique](#poor-technique)
  - [Split BT](#split-bt)
  - [Triple BT](#triple-bt)
- [Techniques](#techniques)
  - [Chords](#chords)
    - [Chord Jacks](#chord-jacks)
    - [Chord Stairs](#chord-stairs)
      - [Static Position](#static-position)
    - [Hamburgers](#hamburgers)
  - [Hand-blading](#hand-blading)
  - [Jacks](#jacks)
    - [As One-hand Trills](#as-one-hand-trills)
    - [Static Position](#static-position-1)
  - [Turning Long Lasers](#turning-long-lasers)
    - [Poor Technique](#poor-technique-1)
- [Advanced Techniques](#advanced-techniques)
  - [Chords and Trills with Double FX](#chords-and-trills-with-double-fx)
  - [Fast Stairs](#fast-stairs)
  - [Fast Trills](#fast-trills)
  - [Thumbing](#thumbing)

## Intro

This guide contains examples of hand positions and techniques to hit common patterns. SDVX is a very dynamic game that requires you to move your hands (and sometimes body) away from what is commonly referred to as "standard" or "static" position.

### Standard/Static Position

<Image src="/guides/hand-positioning/standard_position.png" alt="standard position" />

This is the default position you will find yourself in, where finger(s) stay over a single button. Playing a pattern standard/static means your hands do not move from this position. Most of the time this is okay, but for patterns such as [chords](/terminology/patterns#chords) or [jacks](/patterns#jacks) they can be too fast to hit accurately without specific techniques.

## Positioning

### Double FX

<Image src="/guides/hand-positioning/ex_double_fx.png" alt="double fx example" />
<Image src="/guides/hand-positioning/double_fx.png" alt="double fx" />

Use your thumb and ring or pinky finger to hit double FX. Reaching `BT-B` / `BT-C` is easier if you use your pinky.

#### Poor Technique

<Image src="/guides/hand-positioning/double_fx_other.png" alt="double fx other" />

Using the side of your palm makes it harder to accurately use your other fingers if needed. Turning your hand sideways prevents you from pressing other buttons completely.

### Split BT

<Image src="/guides/hand-positioning/ex_split_bt.png" alt="split bt example" />
<Image src="/guides/hand-positioning/split_bt.png" alt="split bt" />

Use either your index and pinky finger or turn your hand outwards to use your thumb and middle/ring finger. The latter may be more comfortable if your hands are small.

### Triple BT

<Image src="/guides/hand-positioning/ex_triple_bt.png" alt="triple bt example" />
<Image src="/guides/hand-positioning/triple_bt.png" alt="triple bt" />

This position applies to both triple BT chords and stairs. Similar to split chords, include your middle/ring or index fingers.

Another other position you might see is to use your entire hand:

<Image src="/guides/hand-positioning/triple_bt_other.png" alt="triple bt other" />

## Techniques

### Chords

The key to properly hitting chord jacks and stairs is to release/raise one hand as fast as possible and not to rush to hit with the other hand. This prevents errors from attempting to press a button down that has not yet been released.

#### Chord Jacks

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_chord_jack.png" alt="chord jack example" />
  <Video src="/guides/hand-positioning/chord_jack.mp4" types="video/mp4" />
</div>

Move one hand inwards and then alternate.
<br/>

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_chord_jack_overhand.png" alt="chord jack overhand example" />
  <Video src="/guides/hand-positioning/chord_jack_overhand.mp4" types="video/mp4" />
</div>

Adjust your upper body to reach one hand over the other and then alternate.

#### Chord Stairs

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_chord_stairs.png" alt="chord stairs example" />
  <Video src="/guides/hand-positioning/chord_stairs.mp4" types="video/mp4" />
</div>

One of your hands will be responsible for two chords and this can be played 2 ways:
  - Hit the middle chord by moving the first hand inwards
  - Hit the middle chord by moving the second hand outwards from the middle

##### Static Position

<Video src="/guides/hand-positioning/chord_stairs_static.mp4" types="video/mp4" />

Playing chord stairs without moving your hands is referred to as playing them standard/static. Since each hand is responsible for pressing one button twice, it becomes too difficult to hit on-time if the stair is fast.

#### Hamburgers

Use one thumb under the other hand or adjust your upper body to reach one hand over the other and then alternate.

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_hamburgers.png" alt="hamburgers example" />
  <Video src="/guides/hand-positioning/hamburgers.mp4" types="video/mp4" />
</div>

### Hand-blading

<Video src="/guides/hand-positioning/handblading.mp4" types="video/mp4"/>

This refers to turning knobs with an open hand. **Not recommended** as it is poor technique (you can quickly run out of room to turn long lasers) and excessive sideways force can damage the knobs.

### Jacks

The key to properly hitting jacks is to release/raise one hand as fast as possible and not to rush to hit with the other hand. This prevents errors from attempting to press a button down that has not yet been released.
<br/>

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_jacks.png" alt="jacks example" />
  <Video src="/guides/hand-positioning/jacks.mp4" types="video/mp4" />
</div>

Alternate your hands.

#### As One-hand Trills

<Video src="/guides/hand-positioning/jacks_one_hand.mp4" types="video/mp4"/>

An uncommon but viable technique is to quickly alternate your fingers instead of hands on a single button. Movement in the video has been exaggerated for clarity.

#### Static Position

<Video src="/guides/hand-positioning/jacks_static.mp4" types="video/mp4" />

Hitting jacks without alternating hands is referred to as playing jacks standard/static. It becomes too difficult to hit on-time if the jack is too fast.


### Turning Long Lasers

<Video src="/guides/hand-positioning/turning_knobs.mp4" types="video/mp4" size="max-w-sm" />

To ensure you do not drop lasers during long turns, alternate between pulling/pushing the knob along with your fingers and and then pushing/pulling with your thumb.

The video demonstrates this with a hand more to the side for clarity but this can also be done with your hand over the knob.

#### Poor Technique

<Video src="/guides/hand-positioning/turning_knobs_poor.mp4" types="video/mp4" size="max-w-sm" />

Turning knobs this way can result in dropped inputs as your hand leaves the knob or running out of room.

## Advanced Techniques

### Chords and Trills with Double FX

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_chord_trills_double_fx.png" alt="chord trills double fx example" />
  <Video src="/guides/hand-positioning/chord_trills_double_fx.mp4" types="video/mp4" />
</div>

Adjust your upper body to reach one hand over the other. Example video is for chords but applies to trills as well.

### Fast Stairs

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_fast_stairs.png" alt="fast stairs example" />
  <Video src="/guides/hand-positioning/fast_stairs.mp4" types="video/mp4" />
</div>

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_fast_stairs_full.png" alt="fast stairs full example" />
  <Video src="/guides/hand-positioning/fast_stairs_full.mp4" types="video/mp4" />
</div>

For BT stairs that are too fast to hit accurately, try alternating hands instead of playing them static. Movement has been exaggerated for clarity.

### Fast Trills

<div className="flex flex-row gap-x-4">
  <Image src="/guides/hand-positioning/ex_fast_trills.png" alt="fast trills example" />
  <Video src="/guides/hand-positioning/fast_trills.mp4" types="video/mp4" />
</div>

For BT trills that are too fast to hit accurately, adjust your body to reach one hand over the other and alternate them every 2 buttons. Movement has been exaggerated for clarity.

### Thumbing

<Video src="/guides/hand-positioning/thumbing.mp4" types="video/mp4" />

This refers to turning knobs and pressing buttons at the same time. Can be done in situations where one-hand patterns become too complex.

## Kuso Techniques

In Japanese, "kuso" (くそ / 糞) literally means "shit". Kuso techniques refer to unorthodox ways to play charts, usually for entertainment. For the minority of playerbase, they can trivialize some very specific patterns.

### Forearm Strats

<Video
  src="/guides/hand-positioning/elbow_strats.mp4"
  types="video/mp4"
  size="max-w-md" 
/>

This refers to using your forearms to alternatively hit quad jacks. Examples include [Ganymede kamome mix EXH 17](https://sdvx.in/01/01071e.htm) and [Gerbera MXM 18](https://sdvx.in/04/mrr04186m.htm)

### Sliding

<Video
  src="/guides/hand-positioning/sliding.mp4"
  types="video/mp4"
  size="max-w-md" 
/>

This refers to using your hands to "slide" across buttons to hit fast stairs. This technique is inefficient and can cause pain.

### One-hand Alternating Jacks

<Video
  src="/guides/hand-positioning/oha_jacks.mp4"
  types="video/mp4"
  size="max-w-md" 
/>

This refers to hitting jacks alternatively but with only one hand. This technique is more useful compared to other kuso strats.
